...
class Downloader:
    ...
    def sanitize_filename(self, filename):
        """
        Sanitizes the given filename by removing or replacing invalid characters, handling leading dots,
        ensuring the resulting filename is not empty, and trimming it to a maximum length of 255 characters.
    ...
    def identify_link_type(self, link):
        """
        Identifies the type of the given link (video, playlist, or unknown) by checking if it is a valid YouTube URL.

        Parameters:
        link (str): The link to identify.

        Returns:
        str: The type of the link: "Video" if it's a video URL, "Playlist" if it's a playlist URL,
            or "unknown" if it's neither a video nor a playlist URL.
        """
    ...
    def DOWNLOAD_STARTER(self, links): # checks if the link is a video or a playlist or if we have a textfile and starts the downloading prosses 
        """
        Checks if the provided links are videos, playlists, or text files and starts the downloading process accordingly.
    ...
    def Save_video_data(self, video_data):
        """
        Saves the details of the song video in the database.

        Parameters:
        video_data (tuple): A tuple containing the details of the song video.
                            The tuple structure should be (title, author, views, length, publish_date, link).
        """
        connection = sqlite3.connect("YT_database.db")
        cursor = connection.cursor()
    ...
    def download_song_and_create_dataframe(self, link, progres_bar=0): 
        """
        Downloads a song from the given YouTube link and converts it to MP3 format.

        Parameters:
        link (str): The YouTube link of the song to download.
        progress_bar (int): (Optional) The progress bar.
        ...
        try:
            ...
            print(f'Song {title} downloaded and converted to MP3 successfully!\n')
            return 
    ...
    def Download_playlist(self, link):
        """
        Downloads and converts a playlist from the given YouTube link to MP3 format.

        Parameters:
        link (str): The YouTube link of the playlist to download.

        Returns:
        list: A list containing details of each song downloaded from the playlist.
        """
        ...
        for video_url in playlist.video_urls:
            ...
            mp3_file = f"{playlist_folder}/{self.pices_count }.{valid_filename}.mp3"
    ...
    def Download_list_of_playlists(self, file):
        """
        Downloads and converts a list of playlists from the given text file to MP3 format.

        Parameters:
        file (str): The path to the text file containing YouTube playlist links.
